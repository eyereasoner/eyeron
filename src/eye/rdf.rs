//! RDF 1.2 N-Quads bridge (spec §13): imports N-Quads as ground
//! `rdf(S, P, O, G)` facts, exports the `rdf/4` relation's answers as
//! N-Quads. Reuses eyeron's existing N-Quads *reader*
//! (`crate::n3::rdf_compat::parse_rdf12`) rather than porting Eyelang's own
//! hand-rolled one; writes one small new serializer for output, since
//! eyeron's existing `triples_to_n3`/`triples_to_trig` writers are
//! Turtle/TriG-shaped, not N-Quads.
...
fn ast_term_to_rdf(value: &AstTerm) -> Result<Term> {
    match value {
        ...
        other => Err(EyeronError::new(format!("Cannot represent {:?} as an Eyelang RDF term", other))),
    }
}
...
/// Render `facts` (each a ground `rdf(S,P,O,G)` `Term::Struct`) as an
/// Eyelang source fragment of ordinary facts — mirrors `nquadsToEyelang`,
/// used to splice `--rdf-input` in as textual facts ahead of the rest of
/// the program.
pub fn facts_to_eyelang(facts: &[Term]) -> String {
                ...
                Term::Struct(dn, dargs) if dn == "language" && (dargs.len() == 1 || dargs.len() == 2) => {
                    ...
                    if dargs.len() == 2 {
                        let dir = match &dargs[1] {
                            Term::Atom(a) if a == "ltr" || a == "rtl" => a.clone(),
                            _ => return Err(bad_term(value)),
                        };
...
pub fn format_nquads(quads: &[Term]) -> Result<String> {
    ...
    for quad in quads {
        ...
        let graph_text = match g {
            Term::Atom(a) if a == "default_graph" => String::new(),
...
/// Render a `--rdf-output` query's answers (from `ask rdf(?subject,
/// ?predicate, ?object, ?graph).`) as N-Quads — mirrors
/// `rdfAnswersToNQuads`.
pub fn answers_to_nquads(query: &QueryResult) -> Result<String> {
    let quads: Vec<Term> = query
        ...
        .iter()
        .map(|answer| {
            let get = |name: &str| answer.bindings.iter().find(|(n, _)| n == name).map(|(_, v)| v.clone()).unwrap_or_else(|| term::atom("undefined"));
