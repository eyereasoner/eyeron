...
- ordinary triple patterns and **property paths**: sequence `/`, inverse `^`, alternative `|`, the length modifiers `?` (zero or one), `+` (one or more) and `*` (zero or more), and `(...)` grouping. A path reports each pair of endpoints it connects once, never its intermediate join nodes, and `+`/`*` terminate on a cyclic graph (a node on a cycle does reach itself). Negated property sets (`!p`) are not part of the grammar;
- `FILTER(expr)`;
...
Prefer `p+`/`p*` over a recursive rule pair when all you need is reachability. A path is walked once per start node as an indexed breadth-first search with a visited set, whereas `RULE { ?x :r ?z } WHERE { ?x :p ?y . ?y :r ?z }` has to be re-scanned against a growing graph until it reaches a fixpoint: over a 600-node chain (180,300 pairs) the path form takes about 4s where the rule pair takes about 175s.
...
1. **`ast.rs`** — `Expr` (the `FILTER`/`SET` expression tree), `PathExpr` (`/`, `^`, `|`, `?`, `+`, `*`), `Clause` (`Triple`/`Path`/`Filter`/`Set`/`Not`), and `SparqlRlRule`/`SparqlRlProgram` — a separate type family from `crate::ast::Rule`/`Document`, precisely because of the ordering constraint above.
...
3. **`eval.rs`** — the ordered clause evaluator described above. `Clause::Not` succeeds iff a recursive `solve_body` call over the negated sub-body finds zero solutions (classic negation as failure); `Clause::Path` is walked by `solve_path`, which defers each individual step to a `PathEnv::step` callback — the forward engine matches a step against its graph index, `backward.rs` proves it as a goal, so a path there may also run over rule-derived edges. `+`/`*` are a breadth-first walk with a visited set (terminating on cycles, and reporting a node on a cycle as reaching itself); the zero-length half of `?`/`*` relates a node to itself, falling back to enumerating the graph's nodes when both endpoints are still unbound.
...
- A `+`/`*` path answers *reachability* over a cyclic graph, but a rule cannot enumerate the individual simple paths that reach a node, nor count their stopovers, the way N3/Eyelang's backward, visited-list-guarded recursion can — a path reports only the pairs of endpoints it connects. `path-discovery.srl` therefore still unrolls one join rule per stopover count up to a fixed bound, each gated by `FILTER(<count> <= ?maxStopovers)`; the source/destination/stopover-count query itself stays fully general, only the *maximum* supported stopover count is fixed at rule-set-authoring time.
