...
3. **`eval.rs`** — the ordered clause evaluator described above. `Clause::Not` succeeds iff a recursive `solve_body` call over the negated sub-body finds zero solutions (classic negation as failure). For forward reasoning, `expand_rule_paths` expands property paths once before stratification and evaluation, so the matcher, activation index, and proof trace use the same triple patterns and join variables. Goal-directed query evaluation can expand a path while solving it.
...
`--proof` output is a genuine `.srl` document, produced by `srl::proof::proof_to_srl` rather than N3's own `proof_to_n3`: N3 writes a proof step as a top-level triple whose subject is the quoted conclusion (`{ s p o } pe:rule 1; ...`), and SRL's grammar has no quoted-graph term (its only quoted-graph-shaped term is the single-triple RDF-star triple term `<<(s p o)>>`). So each step here is instead a small blank node reifying its own conclusion — `_:stepN rdf:reifies <<(s p o)>>` — the idiom `examples/proof-audit.srl` demonstrates by hand, with `pe:uses` linking straight to the blank nodes of the steps it depended on rather than repeating their triples, all inside one `DATA { ... }` block (SRL has no bare top-level triples). The *analysis* that walks every claim's derivation into one flat, deduplicated list of steps is shared verbatim with `n3::proof` (exposed to `srl::proof` as `pub(crate)`); only the SRL-syntax rendering is separate. Underneath, `forward.rs` builds an N3-shaped `Rule` from each `SparqlRlRule` (`premise` = the rule's own positive body patterns, from the same `rule_positive_patterns` the stratifier's activation index already computes; `conclusion` = its head), records one `DerivedFact` per genuinely new fact (not per solution — matching "alternative derivations of an already known answer need not be retained"), and `parser.rs`'s `parse_sparql_rl_with_source` stamps each rule and `DATA {...}` fact with a `SourceRef` the same way `n3::parser`'s own `with_source` does. A step cites the rule it applied as `pe:rule N`, its number in the document — the same citation `n3::proof` makes. A `FILTER`/`NOT`/`SET` clause is not itself reified as a premise (there is no natural `Triple` shape for one) — the trace shows the positive facts that fed the rule and the bindings used, but not, say, a `FILTER` condition's own text; a step's own source line is always one click away from the full rule for that.

## Known limitations

- `--query-mode auto` is not implemented (only `forward`/`backward`).
- The forward reasoner is not as fast as N3's own agenda-based fixpoint for the same shape of problem, though long single-premise rule chains such as `deep-taxonomy-10000.srl` and `deep-taxonomy-100000.srl` are no longer quadratic: dependency computation, rule activation, and match-graph lookups are indexed. Both examples run in the example suite; the 100,000-level case can take tens of seconds when output and proof passes are both included.
- SRL has no backward *rule syntax*. Its `--query-mode backward` evaluates a query against the existing rules without materializing the whole closure. Stratification rejects recursion through negation, so `path-discovery.srl` uses a fixed number of unrolled joins for cycle-safe path search; each is gated by `FILTER(<count> <= ?maxStopovers)`. Queries remain general, but the maximum supported stopover count is fixed when the rule set is written.
