...
/// The three packaged examples that deliberately exercise rejection, and
/// the substring their error message must contain.
const ERROR_EXAMPLES: &[(&str, &str)] = &[
    ("unstratified-negation", "Recursion through \\+ or findall is not stratified"),
    ("variable-predicate-dependency", "Recursion through \\+ or findall is not stratified"),
...
fn example_names() -> Vec<String> {
    ...
    let mut names: Vec<String> = fs::read_dir(&dir)
        ...
        .filter(|path| path.extension().and_then(|ext| ext.to_str()) == Some("pl"))
...
fn source_for(name: &str) -> String {
    read(&manifest_dir().join("examples").join(format!("{name}.pl")))
}

/// Two packaged examples need extra input assembled before running:
/// `proof-audit.pl` replays `socrates.pl`'s own golden proof document as
/// input, and `rdf12-interoperability.pl` reads an RDF 1.2 N-Quads file
/// via the `--rdf-input` bridge.
fn effective_source(name: &str) -> String {
    match name {
        "proof-audit" => {
            let socrates_proof = read(&manifest_dir().join("examples/proof/socrates.pl"));
            format!("{}\n{}", socrates_proof, source_for(name))
        }
        "rdf12-interoperability" => {
            let nquads = read(&manifest_dir().join("examples/rdf12-interoperability.nq"));
            let facts = prolog::rdf::parse_nquads(&nquads, "d0_").unwrap_or_else(|err| panic!("failed to parse rdf12-interoperability.nq: {err}"));
            format!("{}{}", prolog::rdf::facts_to_prolog(&facts), source_for(name))
        }
        _ => source_for(name),
    }
}

fn run_source(source: &str) -> Result<prolog::RunResult, String> {
    prolog::run(source, prolog::Limits::default()).map_err(|err| format!("run failed: {err}\nsource:\n{source}"))
}
...
fn main() {
    ...
    every_proof_golden_reparses_as_a_valid_program();
    cli_behavior_checks();
    ...
    progress_line(&format!(
        "\nprolog result: {}. {total} passed; 0 failed; finished in {elapsed:.2}s ({golden_checked} by golden match, {expected_error} expected-error)",
        green("ok"),
    ));
}

fn report_case(name: &str, status: &str, started: std::time::Instant) {
    progress_line(&format!("example examples/{name}.pl ... {status} ({:.3}s)", started.elapsed().as_secs_f64()));
}
...
fn check_error_example(name: &str, expected_substring: &str) -> Result<(), String> {
    ...
    match prolog::run(&source, prolog::Limits::default()) {
...
fn check_example(name: &str) -> Result<(), String> {
    let plain_golden = manifest_dir().join("examples/output").join(format!("{name}.pl"));
    if !plain_golden.exists() {
        return Err(format!("{name}: every non-error .pl example must have a plain golden, missing {}", plain_golden.display()));
    }
    let proof_golden = manifest_dir().join("examples/proof").join(format!("{name}.pl"));
    if !proof_golden.exists() {
        return Err(format!("{name}: every non-error .pl example must have a proof golden, missing {}", proof_golden.display()));
    }

    let source = effective_source(name);
    let result = run_source(&source)?;

    let actual_plain = prolog::output::format_result(&result, false);
    ...
    let actual_proof = prolog::output::format_result(&result, true);
...
fn every_example_matches_its_plain_and_proof_goldens() -> usize {
    ...
    assert_eq!(checked, expected, "expected every non-error .pl example ({expected} of {total}) to have matching goldens, got {checked}");
    checked
}
...
fn every_proof_golden_reparses_as_a_valid_program() {
    ...
    for entry in fs::read_dir(&proof_dir).unwrap() {
        ...
        let text = read(&path);
        prolog::check(&text).unwrap_or_else(|err| panic!("golden {} does not reparse as a valid program: {err}", path.display()));
    }
}
...
fn cli_behavior_checks() {
    cli_runs_a_dot_pl_file_and_matches_the_golden();
    ...
    cli_query_flag_appends_a_directive();
}

fn cli_runs_a_dot_pl_file_and_matches_the_golden() {
    let path = manifest_dir().join("examples/ancestor.pl");
    let output = run_cli(&[path.to_str().unwrap()]);
    assert!(output.status.success(), "stderr: {}", String::from_utf8_lossy(&output.stderr));
    let golden = read(&manifest_dir().join("examples/output/ancestor.pl"));
...
fn cli_rdf_input_and_rdf_output_round_trip() {
    ...
    let pl_path = manifest_dir().join("examples/rdf12-interoperability.pl");
    let output = run_cli(&["--rdf-input", nq_path.to_str().unwrap(), pl_path.to_str().unwrap()]);
    assert!(output.status.success(), "stderr: {}", String::from_utf8_lossy(&output.stderr));
    let golden = read(&manifest_dir().join("examples/output/rdf12-interoperability.pl"));
    assert_eq!(String::from_utf8_lossy(&output.stdout), golden);

    let round_trip = run_cli(&["--rdf-input", nq_path.to_str().unwrap(), "--rdf-output", pl_path.to_str().unwrap()]);
...
fn cli_rejects_oversized_limits_and_reports_exit_code_two() {
    let path = manifest_dir().join("examples/hanoi.pl");
