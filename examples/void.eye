...
made(?person, ?book) if author(?book, ?person).
subject_of(?person, ?book) if about(?book, ?person), person(?person).
subject(?book, quantumTheory) if about(?book, quantumFieldTheory).
about(?book, darkEnergy) if about(?book, cosmologicalConstant).
recommended_for(?book, curiousPhysicsReaderEn) if subject(?book, quantumTheory), in_language(?book, en).

ask made(?person, ?book).
ask subject_of(?person, ?book).
ask subject(?book, ?topic).
ask recommended_for(?book, ?reader).
